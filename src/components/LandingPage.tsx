...
function ExecutionMockup() {
  const milestones = [
    { name: "Mobilisation",            pct: 100, done: true,  tasks: [
        { name: "Site survey",    start: "01 Jun", end: "05 Jun", pct: 100 },
        { name: "Team briefing",  start: "06 Jun", end: "08 Jun", pct: 100 },
      ],
    },
    { name: "Preliminaries",           pct: 100, done: true,  tasks: [] },
    { name: "Procurement",             pct: 72,  done: false, tasks: [
        { name: "RFQ issued",     start: "10 Jun", end: "20 Jun", pct: 100 },
        { name: "Bid evaluation", start: "21 Jun", end: "05 Jul", pct: 44  },
      ],
    },
    { name: "Installation",            pct: 40,  done: false, tasks: [] },
    { name: "Testing & Commissioning", pct: 0,   done: false, tasks: [] },
    { name: "Handover",                pct: 0,   done: false, tasks: [] },
  ];
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-lg overflow-hidden select-none">
      <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50 flex-wrap gap-1">
        <div className="flex items-center gap-2 flex-wrap">
          <Activity className="w-3.5 h-3.5 text-blue-600 flex-shrink-0" />
          <span className="text-[11px] font-semibold text-slate-800">Execution Milestones</span>
          <span className="flex items-center gap-1 text-[10px] text-slate-500 bg-white border border-slate-200 rounded-full px-2 py-0.5">
            <CalendarDays className="w-2.5 h-2.5 text-slate-400" />
            01 Jun 26 → 15 Mar 27
          </span>
        </div>
        <span className="text-[10px] bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-full font-bold flex-shrink-0">
          {overall}%
        </span>
      </div>
      {/* Overall bar */}
      ...
      {/* Milestone rows */}
      <div className="px-4 pb-3 space-y-1.5 mt-0.5">
        {milestones.map(m => (
          <div key={m.name}>
            <div
            ...
            >
              <div className={`w-3 h-3 rounded-full flex-shrink-0 flex items-center justify-center ${
                m.done ? "bg-emerald-500" : m.pct > 0 ? "bg-blue-500" : "bg-slate-200"
              }`}>
                {m.done && <CheckCircle2 className="w-2 h-2 text-white" />}
              </div>
              <span className={`text-[10px] font-medium w-28 flex-shrink-0 truncate ${m.done ? "text-emerald-600 line-through" : m.pct > 0 ? "text-slate-700" : "text-slate-400"}`}>
                {m.name}
              </span>
              <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${m.done ? "bg-emerald-500" : "bg-blue-500"}`} style={{ width: `${m.pct}%` }} />
              </div>
              <span className={`text-[10px] font-semibold w-7 text-right flex-shrink-0 ${m.done ? "text-emerald-600" : m.pct > 0 ? "text-blue-600" : "text-slate-300"}`}>
                {m.pct}%
              </span>
              {m.tasks.length > 0 && (
                <ChevronRight className={`w-2.5 h-2.5 text-slate-300 flex-shrink-0 transition-transform ${expanded === m.name ? "rotate-90" : ""}`} />
              )}
            </div>

            {/* Expanded subtasks */}
            {expanded === m.name && m.tasks.length > 0 && (
              <div className="ml-5 mt-1.5 space-y-1">
                {m.tasks.map(t => (
                  <div key={t.name} className="bg-slate-50 border border-slate-100 rounded px-2 py-1.5">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[9px] font-medium text-slate-700 flex-1 truncate">{t.name}</span>
                      <span className="flex items-center gap-0.5 text-[9px] text-slate-400 flex-shrink-0">
                        <CalendarDays className="w-2 h-2" />{t.start} → {t.end}
                      </span>
                      <span className={`text-[9px] font-bold flex-shrink-0 ${t.pct === 100 ? "text-emerald-600" : "text-blue-600"}`}>{t.pct}%</span>
                    </div>
                    <div className="h-1 bg-slate-200 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${t.pct === 100 ? "bg-emerald-500" : "bg-blue-500"}`} style={{ width: `${t.pct}%` }} />
                    </div>
                  </div>
                ))}
              </div>
...
function MilestoneTaskMockup() {
  const tasks = [
    { name: "Structural survey",   start: "02 Jun 26", end: "08 Jun 26", pct: 100 },
    { name: "Mobilise equipment",  start: "09 Jun 26", end: "20 Jun 26", pct: 65  },
    { name: "Safety induction",    start: "02 Jun 26", end: "03 Jun 26", pct: 100 },
  ];
  const avg = Math.round(tasks.reduce((s, t) => s + t.pct, 0) / tasks.length);
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-lg overflow-hidden select-none text-left">
      {/* Header */}
      <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
        <div className="flex items-center gap-1.5 flex-wrap">
          <Activity className="w-3 h-3 text-blue-600 flex-shrink-0" />
          <span className="text-[10px] font-semibold text-slate-800">Mobilisation</span>
          <span className="text-[9px] bg-slate-100 rounded px-1.5 py-0.5 text-slate-500">3 tasks</span>
          <span className="flex items-center gap-0.5 text-[9px] text-slate-500 bg-white border border-slate-200 rounded-full px-1.5 py-0.5 ml-1">
            <CalendarDays className="w-2 h-2 text-slate-400" />
            02 Jun → 20 Jun 26
          </span>
        </div>
        <span className="text-[10px] font-bold text-blue-600 flex-shrink-0">{avg}%</span>
      </div>
      {/* Milestone bar */}
      <div className="px-4 pt-2.5 pb-2">
        <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
        ...
        <p className="text-[9px] text-slate-400 mt-1">Auto-computed from subtasks</p>
      </div>
      {/* Task rows */}
      <div className="px-4 pb-3 space-y-1.5">
        {tasks.map(t => (
          <div key={t.name} className="bg-slate-50 border border-slate-100 rounded-lg px-2.5 py-2">
            <div className="flex items-center gap-2 mb-1.5">
              <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${t.pct === 100 ? "bg-emerald-400" : "bg-blue-400"}`} />
              <span className="text-[10px] font-medium text-slate-700 flex-1 truncate">{t.name}</span>
              <span className="flex items-center gap-0.5 text-[9px] text-slate-400 flex-shrink-0 whitespace-nowrap">
                <CalendarDays className="w-2 h-2" />{t.start}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[9px] font-bold w-6 text-right flex-shrink-0 ${t.pct === 100 ? "text-emerald-600" : "text-blue-600"}`}>{t.pct}%</span>
            </div>
          </div>
        ))}
      </div>
          ...
          <div className="flex-1 space-y-5">
            ...
            <ul className="space-y-3">
              {[
                ...
                "Break milestones into subtasks with start and end dates",
                "Milestone progress auto-computes from subtask averages",
                "Date timelines roll up: tasks → milestone → package → project",
